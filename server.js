...
async function runSeedIfNeeded() {
  try {
    const pool = require('./backend/database/db');
    const [hasOldBanner] = await pool.query("SELECT COUNT(*) as cnt FROM banners WHERE image LIKE '%32d65.png'");
    if (hasOldBanner[0].cnt > 0) {
      console.log(`Found ${hasOldBanner[0].cnt} banners with old colliding path, re-running seed...`);
